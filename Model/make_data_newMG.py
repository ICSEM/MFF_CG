...
def make_nl_vocab():
    ...
    i = 3
    with open("MG_datasets/newnls_1000.txt", 'r', encoding='UTF-8') as f:
        lines = f.readlines()
        for line in lines:
            line= line.split('\n')
            line1 = line[0]
...
def make_code_vocab():
    w2i_code = {"pad": 0, "SOS": 1, "EOS": 2, "(":3, ")":4 , ".":5}
    i = 6
    with open("MG_datasets/preMG_code_1000.txt", 'r', encoding='UTF-8') as f:
        ...
        for line in lines:
            ...
            # str1= ''.join(line1)
            # print(str1)
            # line_tok = re.split('[^\w]+',str1)
            line_tok = line1.replace('(',' ').replace(')',' ').replace('.',' ')
            line_tok = line_tok.split()
            # len1 = len(line_tok)
            # len1 = len1 - 1
            # line_tok = line_tok[1:len1]
            # print(line_tok)
            # exit()
            for w in line_tok:
...
def generate_code_datasets():
    ...
    with open("MG_datasets/preMG_code_1000.txt", 'r',encoding='UTF-8') as f:
        ...
        for line in lines:
            i=i+1
            line1 = line.split('\n')
            line2 = line1[0].replace('(',' ( ').replace(')',' ) ').replace('.',' . ')
