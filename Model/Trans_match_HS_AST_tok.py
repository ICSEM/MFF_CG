...
from make_data_newHS598 import make_nl_vocab
from make_data_newHS598 import make_code_vocab
from make_data_newHS598 import generate_sentences
from make_data_newHS598 import generate_code_datasets
from make_data_newHS598 import generate_nl_datasets
from Generate_AST_num import generate_AST_num
from match_HS_AST_tok import match_Tok_AST2
...
class Decoder(nn.Module):
    def __init__(self):
        ...
        for i1 in range(AST_num):
            ...
            features1 = get_embed('HS_AST/'+file_name).to(Device)
            adj1=get_A('HS_AST/'+file_name).to(Device)
            tgt_emb1 = net1(features1, adj1)

            print("执行完GCN嵌入的序号：",i1)

            AST_order = get_AST_order('HS_AST/' + file_name)
    ...
    def forward(self, dec_inputs, enc_inputs, enc_outputs): # dec_inputs : [batch_size x target_len]
        ...
        if dec_inputs.shape[0] == 1:
            ...
            # print(rela[0][0])
            batch_order = 0
            for AST_order in range(dec_test_emb,dec_test_emb+4):
...
if __name__ == '__main__':
    ...
    torch.save(net1.state_dict(), 'save_model1/HSgcn_AST_model4_4_598_300.pt')
    # torch.save(net2.state_dict(), 'save_model/newgcn_ADG_model3_8_2500_300.pt')
    torch.save(model.state_dict(), 'save_model1/HStrans_model4_4_598_300.pt')
    ...
    with open('predict_source_code/HSoutput4_4_598_300.txt', 'w', encoding='utf-8') as f1:
        f1.write('')
    with open('predict_source_code/HSoutput4_target_4_598_300.txt', 'w', encoding='utf-8') as f2:
        f2.write('')
    ...
    for j in range(j1, len(sentences)):
        model.load_state_dict(torch.load('save_model1/HStrans_model4_4_598_300.pt'))
        ...
        str2 = sentences[j][2]
        with open('predict_source_code/HSoutput4_4_598_300.txt', 'a', encoding='utf-8') as f1:
            f1.write(str1)
            f1.write('\n')
        with open('predict_source_code/HSoutput4_target_4_598_300.txt', 'a', encoding='utf-8') as f2:
            f2.write(str2)
            f2.write('\n')
