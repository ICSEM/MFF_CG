...
class Decoder(nn.Module):
    def __init__(self):
        ...
        self.tgt_emb4 = tgt_emb2.to(Device)
    ...
    def forward(self, dec_inputs, enc_inputs, enc_outputs): # dec_inputs : [batch_size x target_len]
        ...
        if dec_inputs.shape[0] == 1:
            ...
            for AST_order in range(dec_test_emb,dec_test_emb+8):
                ...
                print(dec1.shape)
                print(dec2.shape)
                for k,v in rela1[AST_order][0].items():
                    v = int(v)
                    if k < 400 and v < 400:
                        dec1[batch_order1][k] = dec1[batch_order1][k] + dec2[v]
                batch_order1 = batch_order1 + 1
            ...
            for ADG_order in range(dec_test_emb, dec_test_emb + 4):
                for i in range(ADG_find_pos, j2):
                    node_tok = rela2[i].split('+')
                    ...
                    if ADG_order == node_tok_order:
                        ...
                        if node_tok_start < 400 and node_tok_end < 400:
                            for i1 in range(node_tok_start, node_tok_end):
                                dec1[batch_order2][i1] = dec1[batch_order2][i1] + dec3[i]
                    else:
                        ADG_find_pos = i
                        batch_order2 = batch_order2 + 1
                        break
