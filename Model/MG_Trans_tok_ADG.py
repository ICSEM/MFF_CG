...
from match_Tok_ADG2 import match_Tok_ADG2
...
class Decoder(nn.Module):
    def __init__(self):
        ...
        features2 = get_embed("MG_datasets3/preADG2.txt").to(Device)
        adj2 = get_A("MG_datasets3/preADG2.txt").to(Device)
        tgt_emb2 = net2(features2, adj2)
        print(tgt_emb2.shape)

        # print(tgt_emb1)
        self.tgt_emb2=tgt_emb2.to(Device)
    ...
    def forward(self, dec_inputs, enc_inputs, enc_outputs): # dec_inputs : [batch_size x target_len]
        ...
        if dec_inputs.shape[0] == 1:
            ...
            batch_order = 0
            dec2 = self.tgt_emb2
            # print("初始一个批次的token的嵌入向量:",dec1[0][155])
            # print("ADG的嵌入向量",dec2)
            for ADG_order in range(dec_test_emb,dec_test_emb+4):
                for i in range(j1,j2):
                    node_tok = rela[i].split('+')
                    node_tok_order = int(node_tok[0])
                    if ADG_order == node_tok_order:
                        node_tok_start = int(node_tok[1])
                        node_tok_end = int(node_tok[2])
                        if node_tok_start < 400 and node_tok_end < 400:
                            for i1 in range(node_tok_start,node_tok_end):
                                # print('初始：')
                                dec1[batch_order][i1] = dec1[batch_order][i1] + dec2[i]
                    else:
                        j1 = i
                        batch_order =batch_order + 1
                        break
            # print("融合之后一个批次token的嵌入向量:",dec1[0][155])
            # exit()
...
if __name__ == '__main__':
    ...
    loader = Data.DataLoader(MyDataSet(enc_inputs1, dec_inputs1, target_batch1), 4, True)
    ...
    for epoch in range(300):
        ...
        for enc_inputs, dec_inputs, target_batch in loader:
            ...
            # loss.backward()
            optimizer.step()
            dec_test_emb = dec_test_emb+4

    # torch.save(net1.state_dict(), 'save_model1/Tgcn_AST_model5_4_500_300.pt')
    torch.save(net2.state_dict(), 'save_model/Tgcn_ADG_model5_4_500_300.pt')
    torch.save(model.state_dict(), 'save_model1/Ttrans_model5_4_500_300.pt')
    ...
    with open('predict_source_code/Toutput5_4_500_300.txt', 'w', encoding='utf-8') as f1:
        f1.write('')
    with open('predict_source_code/Toutput5_target_4_500_300.txt', 'w', encoding='utf-8') as f2:
        f2.write('')
    ...
    for j in range(j1, len(sentences)):
        model.load_state_dict(torch.load('save_model1/Ttrans_model5_4_500_300.pt'))
        ...
        str2 = sentences[j][2]
        with open('predict_source_code/Toutput5_4_500_300.txt', 'a', encoding='utf-8') as f1:
            f1.write(str1)
            f1.write('\n')
        with open('predict_source_code/Toutput5_target_4_500_300.txt', 'a', encoding='utf-8') as f2:
            f2.write(str2)
            f2.write('\n')
